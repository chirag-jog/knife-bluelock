      ...
      def run
        ...
        for vapp in vapps
          vapp.servers.all.each do |server|
            ...
            server_list << server.network_connections[:ExternalIpAddress]
            server_list << server.network_connections[:IpAddress]
            server_list << server.operating_system[:"ovf:Description"]
          end
        end
